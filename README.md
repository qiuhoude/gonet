...
服务器之间rpc，客户端服务器之间json + rpc，客户端tcp遵从如下消息包头

    前四位 json name 的 crc，中间json字节流， 尾部+结束标志💞♡ (结束标志也可以自己定义在base.TCP_END控制)
    //另外支持包头大小- 前四位包体大小,再四位json name 的 crc，中间jsonf字节流,代码注视掉,（搜索tcp粘包固定包头）
